...
$(function () {
    ...
    table = $('.ajax-data-table').DataTable({
        ...
        columnDefs: [{
            ...
            "render": function (data, type, row, meta) {
                return `<a class="avatar-group image-popup" href="/page/` + data + `">
        ...
        },
        {
            "render": function (data, type, row, meta) {
                ...
                html += `<a href="/admin/Page/Upsert?id=` + data + `"  class="btn text-white btn-warning"><i class="fa fa-edit "></i></a></div>`;
...
function DeleteItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Page/DisablePages?Id=' + id,
...
function RestoreItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Page/ActivePage?Id=' + id,
                beforeSend: function () { $("button").prop("disabled", true); },
                complete: function () { $("button").prop("disabled", false); },
                success: function (data) {
