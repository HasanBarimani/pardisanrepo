...
$(function () {
    ...
    table = $('.ajax-data-table').DataTable({
        ...
        {
            "render": function (data, type, row, meta) {
                let html = `<div class="btn-group mt-3" role="group">`;
                ...
                html += `<a href="/admin/News/Upsert?id=` + data + `"  class="btn text-white btn-warning"><i class="fa fa-edit "></i></a></div>`;
...
function DeleteItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/News/DisableNews?Id=' + id,
...
function RestoreItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/News/ActiveNews?Id=' + id,
                beforeSend: function () { $("button").prop("disabled", true); },
                complete: function () { $("button").prop("disabled", false); },
                success: function (data) {
