    ...
    table = $('.ajax-data-table').DataTable({
        ...
        },
        {
            "render": function (data, type, row, meta) {
                ...
                html += `<a href="/admin/Holding/Upsert?id=` + data + `"  class="btn text-white btn-warning"><i class="fa fa-edit "></i></a></div>`;
                html += `<a href="/admin/Holding/Gallery?id=` + data + `"  class="btn text-white btn-warning"><i class="fa fa-image "></i></a></div>`;
...
function DeleteItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Holding/DisableBlog?Id=' + id,
...
function RestoreItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Holding/ActiveBlog?Id=' + id,
                beforeSend: function () { $("button").prop("disabled", true); },
                complete: function () { $("button").prop("disabled", false); },
                success: function (data) {
